...
export default function Home() {
  ...
  const bottomCodePanelRef = useRef<any>(null);
            ...
            <div className="h-16 border-b flex items-center px-4 gap-6 bg-background shrink-0 overflow-x-auto">
                ...
                <div className="flex flex-col gap-1 justify-center shrink-0">
                    <div className="flex items-center gap-2">
                        ...
                        <Switch 
                           checked={trigger.enabled && trigger.key === activeYChannel} 
                           onCheckedChange={(c) => {
                               const newT = { ...trigger, enabled: c, key: activeYChannel, waiting: c };
                               setTrigger(newT); triggerRef.current = newT;
                               if (c) { setIsPaused(false); isPausedRef.current = false; }
                           }} 
                           className="scale-75" 
                        />
                        {trigger.enabled && trigger.key === activeYChannel && trigger.waiting && <span className="text-[10px] text-destructive animate-pulse ml-2 font-bold uppercase tracking-widest">Waiting</span>}
                    </div>
                    <div className="flex items-center gap-1 opacity-80">
                        <Select 
                           value={trigger.key === activeYChannel ? trigger.mode : "Rising Edge"} 
                           disabled={!trigger.enabled || trigger.key !== activeYChannel}
                           onValueChange={(v) => { const newT = { ...trigger, mode: v || "" }; setTrigger(newT); triggerRef.current = newT; }}
                        >
            ...
            <div className="h-12 border-b flex items-center px-4 gap-4 shrink-0 bg-background">
                ...
                <Dialog>
                  <DialogTrigger asChild>
                    <Button size="sm" variant="default" className="font-bold"><Download className="w-4 h-4 mr-2" /> Firmware Update</Button>
                  </DialogTrigger>
...
print("Rewrote page.tsx with workspace paradigm successfully.")
