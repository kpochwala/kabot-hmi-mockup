...
`;
  ...
  // Domains
  const yMin = yOffset - yScale / 2;
  const yMax = yOffset + yScale / 2;
                ...
                <TabsContent value="data" className="h-full p-0 m-0 outline-none data-[state=inactive]:hidden">
                  <ResizablePanelGroup orientation="horizontal" className="h-full bg-muted/5">
                      <ResizablePanel defaultSize={20} minSize={15} className="p-4 flex flex-col overflow-y-auto border-r bg-background">
                        <div className="text-[10px] text-muted-foreground uppercase tracking-wider mb-4 shrink-0 font-bold">Signal Browser</div>
                      ...
                      <ResizablePanel defaultSize={80} minSize={50} className="flex flex-col">
                        <div className="h-16 border-b flex items-center px-4 gap-6 bg-background shrink-0">
                            <div className="flex items-center gap-2">
                                <Button 
                            ...
                            <div className="flex items-center gap-4">
                                ...
                                <Knob value={yScale} min={0.1} max={20} step={0.1} onChange={setYScale} label="Volts/Div" />
                                <Knob value={yOffset} min={-10} max={10} step={0.1} onChange={setYOffset} label="Offset" />
                            </div>
                            <div className="w-px h-8 bg-border" />
                        ...
                        <div className="flex-1 min-h-0 relative bg-black p-4 select-none" ref={plotContainerRef}>
                          {yCursor1 !== null && (
                              <div 
                                  ...
                                  <div 
                                     ...
                                     onPointerDown={(e) => {
                                         ...
                                         const move = (ev: PointerEvent) => {
                                             const y = ev.clientY - rect.top;
                                             const pct = 1 - (y / rect.height);
                                             setYCursor2(yMin + pct * (yMax - yMin));
                                         };
                                         const up = () => { window.removeEventListener('pointermove', move); window.removeEventListener('pointerup', up); };
                                         window.addEventListener('pointermove', move); window.addEventListener('pointerup', up);
                                     }}
                          ...
                          <ResponsiveContainer width="100%" height="100%">
                            <LineChart data={plotHistory}>
                              ...
                              <YAxis domain={[yMin, yMax]} tick={{fontSize: 10, fill: '#888'}} width={30} axisLine={false} tickLine={false} />
                              {plottedKeys.map((key, i) => (
                                 <Line key={key} type="monotone" dataKey={key} stroke={chartColors[i % chartColors.length]} strokeWidth={2} dot={false} isAnimationActive={false} />
                              ))}
                            </LineChart>
                          </ResponsiveContainer>
                          ...
                          <div className="absolute top-2 right-16 flex flex-col items-end pointer-events-none">
                              ...
                              {yCursor1 !== null && yCursor2 !== null && <div className="text-white text-[10px] font-mono mt-1">ΔY: {Math.abs(yCursor1 - yCursor2).toFixed(3)}</div>}
                          </div>
                        </div>
                      </ResizablePanel>
                  </ResizablePanelGroup>
